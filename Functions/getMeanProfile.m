function [ Mean, Std, N, Profiles, FluoMean, CellWidth, CellLength, CellID, PixSize] = getMeanProfile( Path, k, NbUpSamples, FieldName, Cell_Length, cutup, fluo)
...
%   Detailed explanation goes here
    %% Loading data
        ...
        %% Check for emtpy cells
        NbFrames=max(size(DataSet.frames));
        delete = zeros(1,NbFrames);
        for i=1:NbFrames
            delete(i) = isempty(DataSet.frames(i).cells);
        end
        delete_rows = find(delete);
        DataSet.frames(delete_rows) = [];
            ...
            for i=1:NbCells(h)
                if cutup
                    if((~DataSet.frames(h).cells.Stats(i).CellDeleted) && (DataSet.frames(h).cells.Stats(i).CellLength)< LenInPix)
                        ...
                        FluoMean(n)=DataSet.frames(h).cells.Stats(i).(strcat('MeanCellIntensity_',fluo));
